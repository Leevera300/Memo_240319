...
public class PostBO {
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		if (prevId != null) { // 2) 이전
			...
			Collections.reverse(postList); // void = 뒤집고 저장까지 해준다
			return postList;
	...
	// input: postId, subject, content, file
	// output: void
	public void updatePostbyPostId(int userId, String loginId, 
			int postId, String subject, String content, 
			MultipartFile file) {
		
		// 기존글 가져온다. (1. 이미지 교체시 삭제하기 위해	2. 업데이트 대상이 있는지 확인)
		Post post = postMapper.selectPostByUserIdAndPostId(userId, postId);
		if (post == null) {
		...
		if (file != null) {
			...
			// 업로드 성공 시 (null 아님) 그리고 기존 이미지가 있으면 제거
